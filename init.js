var SubjectObj = [
    {
        id: "subject1",
        name: "法規課程",
        info: "政府採購法之總則、招標及決標：是非 12 題；選擇 24 題<br>政府採購法之履約管理及驗收：是非 2 題；選擇 4 題<br>政府採購法之罰則及附則：是非 5 題；選擇 10 題<br>政府採購法之爭議處理：是非 4 題；選擇 8 題<br>是非：1 分/題；選擇：2 分/題",
        wsurl_MC: "https://script.google.com/macros/s/AKfycbw0cUYsP3gh3ypDIGIYjgoyszlsKvX88BjWSsw6urE7pYFNdho/exec",
        wsurl_TF: "https://script.google.com/macros/s/AKfycbw0cUYsP3gh3ypDIGIYjgoyszlsKvX88BjWSsw6urE7pYFNdho/exec"
    },
    {
        id: "subject2",
        name: "實務課程",
...
function Quiz_MultipleChoice(id, question, option1, option2, option3, option4, answer, detail) {
    this.id = id;
    ...
    $.getJSON(url, function (json_data) {
        ...
        $.each(json_data, function (key, val) {
            ...
            if (type == "MC") {
                ...
                var tmp = new Quiz_MultipleChoice("Q" + val.index, val.data[2], val.data[3], val.data[4], val.data[5], val.data[6], val.data[1], "");
                QuizArr.push(tmp);
                var temphtml = '<div id="' + tmp.id + '" class="ui form">\
                              <div class="grouped fields">\
                                ...
                                <div class="field">\
                                  <div class="ui radio checkbox">\
                                    <input type="radio" name="' + tmp.id + '" value="3">\
                                    <label>'+ tmp.option3 + '</label>\
                                  </div>\
                                </div>\
                                <div class="field">\
                                  <div class="ui radio checkbox">\
                                    <input type="radio" name="' + tmp.id + '" value="4">\
                                    <label>'+ tmp.option4 + '</label>\
                                  </div>\
                                </div>\
                              </div>\
                ...
                var tmp = new Quiz_TrueFalse("Q" + val.index, val.data[2], val.data[3], val.data[4], val.data[1], "");
                QuizArr.push(tmp);
                var temphtml = '<div id="' + tmp.id + '" class="ui form">\
                              <div class="grouped fields">\
                                <label>'+ tmp.id + '. ' + tmp.question + '</label>\
                                ...
                                <div class="field">\
                                  <div class="ui radio checkbox">\
                                    <input type="radio" name="' + tmp.id + '" value="0">\
                                    <label>'+ tmp.option2 + '</label>\
                                  </div>\
                                </div>\
                              </div>\
...
function submitQuiz() {
    if ($("#quiztable input:checked").length == QuizArr.length) {
            ...
            QuizArr.forEach(function (item, index) {
                if ($("#" + item.id + " input:checked").length == 0) {
                    ...
                    if (useranswer == realanswer) {
                        ...
                        $("#" + item.id + " .detailanswer").html("正確<br>詳解：<br>" + item.detail)
                        score += 2;
                    } else {
        ...
        QuizArr.every(function (item, index, arry) {
            if ($("#" + item.id + " input:checked").length == 0) {
                $('html, body').animate({
                    scrollTop: $("#" + item.id).offset().top - $("#subjecttitle").offset().top
                }, 1000);
            } else {
                return true
            }
...
function reQuiz() {
    $("#quizscore").html("");
    $("#reQuiz").hide();
    getQuestions($("#subjecttitle").attr("qvalue"), $("#subjecttitle").attr("qtype"));
}